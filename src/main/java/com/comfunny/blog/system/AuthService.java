	...
	public Params saveMenuAuth(Params inParams) {
		...
		for (DataRow saveDr :menuAuthDt) {
			for (DataRow colDr :columnsDt) {
				...
				colDr.setParam("authYn", saveDr.getVal(CommUtil.strCamel(authCode)));
				colDr.setParam("authCd", authCode);
				colDr.setParam("menuSeq", saveDr.getVal("menuSeq"));
				colDr.setParam("authGroupSeq", authGroupSeq);
				colDr.setParam("s_userId", s_userId);
